---
title: "EDA"
author: "Kylie Wheelock Riley"
date: "10/3/2019"
output: html_document
---

```{r setup, include=FALSE}
library(tidyverse)
library(viridis)

knitr::opts_chunk$set(
  echo = TRUE,
  warning = FALSE,
  fig.width = 8, 
  fig.height = 6,
  out.width = "90%")

options(
  ggplot2.continuous.colour = "viridis",
  ggplot2.continuous.fill = "viridis"
)
scale_colour_discrete = scale_colour_viridis_d
scale_fill_discrete = scale_fill_viridis_d
theme_set(theme_minimal() + theme(legend.position = "bottom"))

```

```{r}
weather_df = 
  rnoaa::meteo_pull_monitors(c("USW00094728", "USC00519397", "USS0023B17S"),
                      var = c("PRCP", "TMIN", "TMAX"), 
                      date_min = "2017-01-01",
                      date_max = "2017-12-31") %>%
  mutate(
    name = recode(id, USW00094728 = "CentralPark_NY", 
                      USC00519397 = "Waikiki_HA",
                      USS0023B17S = "Waterhole_WA"),
    tmin = tmin / 10,
    tmax = tmax / 10,
    month = lubridate::floor_date(date, unit = "month")) %>%
  select(name, id, date, month, everything())
```

## 'group_by' and counting
```{r}
weather_df %>% 
  group_by(name, month)
```

```{r}
## n_disctinct shows the humber of unique variables
weather_df %>% 
  group_by(name) %>% 
  summarize(n_obs = n(),
            n_unique = n_distinct(date))
```

```{r}
##If you just want a count of things you can go straight to count, no need to group and summarize
weather_df %>% 
  count(name)

weather_df %>% 
  count(month)

weather_df %>% 
  count(name, month)

## can also use name in count, the default is n
```

creating a table using r markdown format
```{r}
##making a nice table to export
weather_df %>% 
  count(name) %>%
  knitr::kable()
```

## 2x2 tables 
AFTER CLASS LOOK UP REST OF LONG CODE
```{r}
weather_df %>% 
  filter(name != "Waikiki_HA") %>% 
  mutate(
    cold = case_when(
      tmax < 5 ~ "cold", 
      tmax >= 5 ~ "not cold", 
      TRUE      ~ "")
  )%>% 
  group_by(name, cold) %>% 
  count() %>% 
  pivot_wider(
    names_from 
  )


```

```{r}
weather_df %>% 
  filter(name != "Waikiki_HA") %>% 
  mutate(
    cold = case_when(
      tmax < 5 ~ "cold", 
      tmax >= 5 ~ "not cold", 
      TRUE      ~ "")
  )%>%
  janitor::tabyl(name, cold)

```

## general summaries
```{r}
weather_df %>% 
  group_by(name) %>% 
  summarize(
    n = n(),
    mean_tmax = mean(tmax, na.rm = TRUE),
    sd_max = sd(tmax, na.rm = TRUE), 
    median_prcp = median(prcp, na.rm = TRUE)
  )
## There are NAs in the dataset, r will return NA/NAN because you cant take the mean of nothing, need to remove the NAs (na.rm = TRUE)
```

```{r}
weather_df %>% 
  group_by(name, month) %>% 
  summarize(
    n = n(),
    mean_tmax = mean(tmax, na.rm = TRUE),
    sd_max = sd(tmax, na.rm = TRUE), 
    median_prcp = median(prcp, na.rm = TRUE))
```

##general summaries

```{r}
weather_df %>% 
  group_by(name) %>% 
  summarize(
    n = n(),
    mean_tmax = mean(tmax, na.rm = TRUE),
    sd_max = sd(tmax, na.rm = TRUE), 
    median_prcp = median(prcp, na.rm = TRUE))
```

## pivot to make more reader friendly summaries
```{r}
weather_df %>% 
  group_by(name) %>% 
  summarize(mean_tmax = mean(tmax)) %>% 
pivot_wider(
  names_from = name, 
  values_from = mean_tmax) %>% 
  knitr::kable(digits = 1)
  
```

## dont forget you can ungroup 
```{r}
weather_df %>% 
  group_by(name) %>% 
  ungroup()
```
 
## group with mutate
```{r}
weather_df %>% 
  group_by(name) %>% 
  mutate(
    mean_tmax = mean(tmax, na.rm = TRUE),
    centered_tmax = tmax - mean_tmax
  ) %>% 
  ggplot(aes(x = date, y = centered + tmax, color = name)) +
  geom_point

```

## window functions in grouped mutate
```{r}
weather_df %>% 
  group_by(name, month) %>% 
  mutate(
    tmax_rank = min_rank(tmax)
  ) %>% 
  filter(tmax_rank == 1)
```

## Lags!

```{r}
weather_df %>% 
  group_by(name) %>% 
  mutate(
    lagged_tmax = lag(tmax),
    one_day_tmx_change = tmax - lagged_tmax
  ) %>% 
summarize(sd_daily_change = sd(one_day_tmx_change, na.rm = TRUE))
```

