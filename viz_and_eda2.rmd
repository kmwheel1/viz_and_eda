...
weather_df %>% 
  mutate(
    name = factor(name),
    name = fct_relevel(name, "Waterhole_WA", "Waikiki_HA", "Central_Park_NY" tmax)
  ) %>% 
  ggplot(aes(x = name, y = tmax, color = name)) + 
  geom_boxplot()
```
...
##restrucutre then plot
sometimes you will need to tidy the data in order to generate the plots you want
```{r}
weather_df %>% 
  pivot_longer(
    tmax:tmin,
